...
public class Guy : MonoBehaviour
{
    ...
    */
    ...
    private void Awake()
    {
        ...
        success.OnEnter += a => { _ent.Stop(); Debug.Log("Plan succeeded"); };

        StateConfigurer.Create(idle).SetTransition(ActionEntity.Kill, kill)
            .SetTransition(ActionEntity.PickUp, pickup).SetTransition(ActionEntity.Open, open).Done();
        StateConfigurer.Create(kill).SetTransition(ActionEntity.NextStep, bridgeStep)
            .SetTransition(ActionEntity.FailedStep, failStep).Done();
        StateConfigurer.Create(pickup).SetTransition(ActionEntity.NextStep, bridgeStep)
            .SetTransition(ActionEntity.FailedStep, failStep).Done();
        StateConfigurer.Create(open).SetTransition(ActionEntity.NextStep, bridgeStep)
            .SetTransition(ActionEntity.FailedStep, failStep).Done();
        StateConfigurer.Create(bridgeStep).SetTransition(ActionEntity.NextStep, bridgeStep)
            .SetTransition(ActionEntity.Success, success).Done();
